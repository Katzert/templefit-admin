  ...
  return (
    <div className="relative z-50 flex items-center" ref={wrapperRef}>
      ...
      <div 
        ...
        className={`flex items-center bg-white/5 border transition-all duration-300 rounded-2xl overflow-hidden cursor-pointer ${
          isOpen ? 'border-temple-gold/60 shadow-[0_0_20px_rgba(212,175,55,0.15)] bg-black/80' : 'border-white/10 hover:border-white/20 hover:bg-white/10'
        }`}
      >
        <div className="pl-3.5 pr-2 text-gray-400">
          <Search size={15} className={isOpen ? 'text-temple-gold' : ''} />
        </div>
        <input 
          ref={inputRef}
          type="text"
          placeholder="Buscar atleta, lead, receta, stock..."
          className="bg-transparent border-none outline-none text-xs text-white placeholder-gray-500 py-2.5 w-48 sm:w-64 md:w-80 font-medium"
          value={query}
          onChange={(e) => {
        ...
        <div className="pr-3 flex items-center gap-1">
          <kbd className="hidden sm:inline-flex items-center gap-0.5 px-2 py-0.5 text-[9px] font-black uppercase text-gray-400 bg-white/10 border border-white/10 rounded-md">
            <Command size={10} /> K
          </kbd>
        </div>
      </div>
      ...
      <AnimatePresence>
        {isOpen && query.trim().length >= 2 && (
          <motion.div
            ...
            className="absolute top-full left-0 md:right-0 mt-3 w-80 sm:w-96 md:w-[480px] bg-[#0E1424] border border-temple-gold/30 rounded-3xl shadow-2xl overflow-hidden max-h-[420px] overflow-y-auto custom-scrollbar z-50 backdrop-blur-2xl"
          >
            {/* Header info */}
            <div className="px-4 py-3 bg-black/40 border-b border-white/10 flex items-center justify-between">
              <span className="text-[10px] font-black uppercase tracking-widest text-temple-gold">
                      ...
                      {searchResults.students.map(st => (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-xl bg-temple-gold/20 text-temple-gold flex items-center justify-center font-bold text-xs">
                              <User size={14} />
                            </div>
                            <div>
                              <p className="text-xs font-black text-white group-hover:text-temple-gold transition-colors leading-tight">{st.name}</p>
                              <p className="text-[10px] text-gray-400 mt-0.5">{st.escuadronId} • {st.plan}</p>
                            </div>
                          </div>
                          <ArrowRight size={12} className="text-gray-600 group-hover:text-temple-gold transition-transform group-hover:translate-x-1" />
                        </button>
                      ))}
                    </div>
                  </div>
                      ...
                      {searchResults.leads.map(ld => (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-xs font-black text-white group-hover:text-blue-400 transition-colors leading-tight">{ld.name}</p>
                              <p className="text-[10px] text-gray-400 mt-0.5">{ld.phone} • Estado: {ld.status}</p>
                            </div>
                          </div>
                          <ArrowRight size={12} className="text-gray-600 group-hover:text-blue-400 transition-transform group-hover:translate-x-1" />
                        </button>
                      ))}
                    </div>
                  </div>
                      ...
                      {searchResults.recipes.map(rc => (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-xs font-black text-white group-hover:text-emerald-400 transition-colors leading-tight">{rc.name}</p>
                              <p className="text-[10px] text-gray-400 mt-0.5">Bs. {rc.suggestedPrice || 0} • {rc.category || 'Nutrición'}</p>
                            </div>
                          </div>
                          <ArrowRight size={12} className="text-gray-600 group-hover:text-emerald-400 transition-transform group-hover:translate-x-1" />
                        </button>
                      ))}
                    </div>
                  </div>
