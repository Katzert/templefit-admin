...
export function ContentMarketingHub() {
  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6 pb-16 font-sans max-w-7xl mx-auto">
      ...
      {/* Header Banner */}
      <motion.div variants={item} className="bg-gradient-to-r from-[#0E1424] via-[#0B0F19] to-black p-6 md:p-8 rounded-3xl border border-white/10 shadow-2xl relative overflow-hidden">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[11px] text-gray-400 font-bold">Matriz de 90 Días Replicable</span>
            </div>
      ...
      <motion.div variants={item} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {resourceFolders.map((folder, idx) => (
          <a
            ...
            className="p-4 bg-[#0E1424]/90 border border-white/10 hover:border-temple-gold/40 rounded-2xl transition group flex items-center justify-between shadow-lg"
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="text-xs font-bold text-white group-hover:text-temple-gold transition line-clamp-1">{folder.name}</h4>
                <p className="text-[10px] text-gray-500 font-medium">{folder.count}</p>
              </div>
            </div>
            <ExternalLink size={14} className="text-gray-500 group-hover:text-white transition" />
          </a>
        ))}
      </motion.div>
      ...
      {hubView === 'annual-roadmap' ? (
        ...
        <motion.div variants={item} className="bg-[#0E1424]/90 border border-white/10 rounded-3xl p-6 md:p-8 shadow-2xl space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-white/10">
            ...
            <span className="px-3.5 py-1.5 bg-black/60 border border-white/10 rounded-xl text-xs font-mono text-emerald-400 font-bold self-start sm:self-auto">
              Horizonte: 12 Meses
            </span>
          </div>
              ...
              <tbody className="divide-y divide-white/5">
                {twelveMonthRoadmap.map((row) => (
                  <tr key={row.month} className="hover:bg-white/5 transition-colors">
                    <td className="py-3.5 px-4 font-black font-mono text-temple-gold whitespace-nowrap">
                      {row.month}
                    </td>
                    <td className="py-3.5 px-4 font-extrabold text-white">
                      <span className="px-2.5 py-1 rounded-xl bg-white/5 border border-white/10 inline-block text-[11px]">
                        {row.phase}
                      </span>
                    </td>
                    <td className="py-3.5 px-4 text-gray-300 font-medium leading-relaxed">
                      {row.activities}
                    </td>
        ...
        <>
          {/* Month Tabs & Controls */}
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-black/60 p-3 rounded-2xl border border-white/10">
            {/* Month Selector */}
            ...
            <div className="relative w-full md:w-72">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 bg-black/50 border border-white/10 rounded-xl text-xs text-white placeholder-gray-500 focus:outline-none focus:border-temple-gold"
              />
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredPosts.map(post => (
          <motion.div key={post.id} variants={item}>
            <Card className="border-white/10 bg-[#0E1424]/90 backdrop-blur-xl shadow-xl hover:border-temple-gold/40 transition flex flex-col justify-between h-full">
              ...
              <CardContent className="space-y-4 flex-1 flex flex-col justify-between pt-0">
                <div className="space-y-3">
                  <div className="p-3.5 bg-black/40 rounded-xl border border-white/5 text-xs text-gray-300 leading-relaxed">
                ...
                <div className="pt-3 border-t border-white/10 flex items-center justify-between gap-2">
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 bg-white/5 hover:bg-white/10 rounded-lg text-xs font-bold text-gray-300 hover:text-white transition flex items-center gap-1.5"
                  >
                    {copiedId === post.id ? (
                      <>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
          <div className="bg-[#0E1424] border border-white/10 rounded-3xl max-w-xl w-full p-6 space-y-4 shadow-2xl">
            ...
            <form onSubmit={handleCreatePost} className="space-y-4 text-xs">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-wider text-gray-400 mb-1">Mes del Plan</label>
                  <select
                    value={newPost.monthIndex}
                    onChange={(e) => setNewPost({ ...newPost, monthIndex: Number(e.target.value) as any })}
                    className="w-full px-3 py-2 bg-black/40 border border-white/10 rounded-xl text-white font-bold"
                  >
                    <option value={1} className="bg-[#121826]">Mes 1</option>
                    <option value={2} className="bg-[#121826]">Mes 2</option>
                    <option value={3} className="bg-[#121826]">Mes 3</option>
                  </select>
                </div>
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-wider text-gray-400 mb-1">Día de Publicación</label>
                  <select
                    value={newPost.dayOfWeek}
                    onChange={(e) => setNewPost({ ...newPost, dayOfWeek: e.target.value as any })}
                    className="w-full px-3 py-2 bg-black/40 border border-white/10 rounded-xl text-white font-bold"
                  >
                    <option value="Lunes" className="bg-[#121826]">Lunes</option>
                    <option value="Martes" className="bg-[#121826]">Martes</option>
                    <option value="Miércoles" className="bg-[#121826]">Miércoles</option>
                    <option value="Jueves" className="bg-[#121826]">Jueves</option>
                    <option value="Viernes" className="bg-[#121826]">Viernes</option>
                    <option value="Sábado" className="bg-[#121826]">Sábado</option>
                    <option value="Domingo" className="bg-[#121826]">Domingo</option>
                  </select>
                </div>
              </div>

              <div>
                <label className="block text-[10px] font-bold uppercase tracking-wider text-gray-400 mb-1">Pilar Temático</label>
                <select
                  value={newPost.pillar}
                  onChange={(e) => setNewPost({ ...newPost, pillar: e.target.value as any })}
                  className="w-full px-3 py-2 bg-black/40 border border-white/10 rounded-xl text-white font-bold"
                >
                  <option value="Storytelling & Testimonios" className="bg-[#121826]">Storytelling & Testimonios</option>
                  <option value="Hábitos 3 Áreas" className="bg-[#121826]">Hábitos 3 Áreas (Cuerpo, Mente, Espíritu)</option>
                  <option value="CristoFit Camp" className="bg-[#121826]">CristoFit Camp & Actividades</option>
                  <option value="Consumo Consciente & Snack" className="bg-[#121826]">Consumo Consciente & Snack Bar</option>
                  <option value="Lives & Retos" className="bg-[#121826]">Lives & Retos VIP (Antonio Eid)</option>
                </select>
              </div>

              <div>
                <label className="block text-[10px] font-bold uppercase tracking-wider text-gray-400 mb-1">Título de la Publicación</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-wider text-gray-400 mb-1">Gancho & Storytelling (Cuerpo del post)</label>
                <textarea
                  rows={4}
                  required
              ...
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-wider text-gray-400 mb-1">Llamada a la Acción (CTA)</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-wider text-gray-400 mb-1">Enlace a Google Drive / PDF / Imagen</label>
                <input
                  type="url"
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-white/10">
                <button
                  ...
                  className="px-4 py-2 bg-white/5 hover:bg-white/10 text-gray-400 hover:text-white rounded-xl font-bold uppercase text-xs"
                >
                  Cancelar
                </button>
                <button
